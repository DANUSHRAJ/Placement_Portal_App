...
class CoursesUpload extends StatefulWidget {
  ...
  CoursesUpload({Key key, this.regnovar, this.usernamevar}) : super(key: key);

//  final AccountsApi api = AccountsApi();
  final CoursesApi api = CoursesApi();

  @override
  _CoursesUploadState createState() =>
      _CoursesUploadState(regnovar: regnovar, usernamevar: usernamevar);
}
...
class _CoursesUploadState extends State<CoursesUpload> {
  ...
  _CoursesUploadState({this.regnovar, this.usernamevar});
...
//      log('Wrong Date');
    ...
    if (check == 1) {
      final upload_Courses = await widget.api.uploadcourse(regnovar, usernamevar,
