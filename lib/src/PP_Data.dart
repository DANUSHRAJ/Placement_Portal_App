...
void profile(List<String> profile){
  ...
  // dob
  // dd/mm/yyyy
  if (!RegExp(r'^([0-2][0-9]|(3)[0-1])(\/)(((0)[0-9])|((1)[0-2]))(\/)\d{4}$').hasMatch(profile[5])) {
    print("dob was invalid");
  }
  // mm/dd/yyyy
  if (!RegExp(r'^(0[1-9]|1[0-2])\/(0[1-9]|1\d|2\d|3[01])\/(19|20)\d{2}$').hasMatch(profile[6])) {
    print("dob was invalid");
  }
  // yyyy/mm/dd
  if (!RegExp(r'^\d{4}\/(0?[1-9]|1[012])\/(0?[1-9]|[12][0-9]|3[01])$').hasMatch(profile[7])) {
    print("dob was invalid");
  }
  ...
  //percentage
  if(!RegExp(r'(^100(\.0{1,2})?$)|(^([1-9]([0-9])?|0)(\.[0-9]{1,2})?$)').hasMatch(profile[9])){
    print("10th percentage");
  }
  if(!RegExp(r'(^100(\.0{1,2})?$)|(^([1-9]([0-9])?|0)(\.[0-9]{1,2})?$)').hasMatch(profile[15])){
    print("10th percentage");
  }
  if(!RegExp(r'(^100(\.0{1,2})?$)|(^([1-9]([0-9])?|0)(\.[0-9]{1,2})?$)').hasMatch(profile[22])){
    print("diploma percentage");
  }
  if(!RegExp(r'(^100(\.0{1,2})?$)|(^([1-9]([0-9])?|0)(\.[0-9]{1,2})?$)').hasMatch(profile[47])){
    print("UG percentage");
  }
  //gpa
  for(int i=26;i<35;i++){
    if(!RegExp(r'(^100(\.0{1,2})?$)|(^([1-9]([0-9])?|0)(\.[0-9]{1,2})?$)').hasMatch(profile[i])){
  ...
  //email id
  if (!RegExp(r'^([a-z0-9\.-]+)@([a-z0-9-]+).([a-z]{2,20})$').hasMatch(profile[56])) {
    print("primary");
  }
  if (!RegExp(r'^([a-z0-9\.-]+)@([a-z0-9-]+).([a-z]{2,20})$').hasMatch(profile[57])) {
    print("alternate");
  }
  if (!RegExp(r'^([a-z0-9\.-]+)@([a-z0-9-]+).([a-z]{2,20})$').hasMatch(profile[67])) {
    print("father email id");
  }
  if (!RegExp(r'^([a-z0-9\.-]+)@([a-z0-9-]+).([a-z]{2,20})$').hasMatch(profile[71])) {
    print("mother email id");
  }

  //pancard
  if (!RegExp(r'[A-Z]{3}[ABCFGHLJPTF]{1}[A-Z]{1}[0-9]{4}[A-Z]{1}$').hasMatch(profile[60])) {
    print("pan card");
  }
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: <Widget>[
                  ...
                  _entryFieldalphabets('NAME OF THE CANDIDATE', 'enter the name with initial', 2),
                  ...
                  _entryFieldalphabets('10TH BOARD OF STUDY', 'Eg:State Board', 10),
                  _entryFieldalphabets('10TH MEDIUM OF STUDY', 'Eg:English', 11),
                  ...
                  _entryFieldalphabets('12th BOARD OF STUDY', 'Eg:STATE BOARD', 16),
                  _entryFieldalphabets('12th MEDIUM OF STUDY', 'Eg:English', 17),
                  ...
                  _entryFieldalphabets('DIPLOMA - SPECIALIZATION/BRANCH', 'state name', 21),
                  _entryFieldnumbers('DIPLOMA Percentage', 'Eg:79.9', 22),
                  _entryFieldnumbers('DIPLOMA YEAR OF PASSING', 'Eg:2017', 23),
                  _entryFieldalphabets('NAME OF INSTITUTE', 'name of institute', 24),
                  ...
                  _entryFieldnumbers('NO OF ARREARS SEM 1', 'if there is no arrears enter 0', 35),
                  _entryFieldnumbers('NO OF ARREARS SEM 2', 'if there is no arrears enter 0', 36),
                  _entryFieldnumbers('NO OF ARREARS SEM 3', 'if there is no arrears enter 0', 37),
                  _entryFieldnumbers('NO OF ARREARS SEM 4', 'if there is no arrears enter 0', 38),
                  _entryFieldnumbers('NO OF ARREARS SEM 5', 'if there is no arrears enter 0', 39),
                  _entryFieldnumbers('NO OF ARREARS SEM 6', 'if there is no arrears enter 0', 40),
                  _entryFieldnumbers('NO OF ARREARS SEM 7', 'if there is no arrears enter 0', 41),
                  _entryFieldnumbers('NO OF ARREARS SEM 8', 'if there is no arrears enter 0', 42),
                  _entryFieldnumbers('TOTAL NO OF STANDING ARREARS', 'if there is no arrears enter 0', 43),
                  _DropBox("HISTORY OF ARREARS [Y/N]", yesorno, 5),
                  _entryFieldnumbers('IF YES, HOW MANY?', 'if there is no arrears enter 0', 44),
                  _entryFieldalphabets('UG DEGREE (FOR PG STUDENTS) ', 'Eg:B.tech', 45),
                  _entryFieldalphabets('UG BRANCH (FOR PG STUDENTS) ', 'Eg:Computer Science', 46),
                  _entryFieldnumbers('UG PERCENTAGE (FOR PG STUDENTS)', 'Eg:95.5', 47),
                  _entryFieldnumbers('UG CGPA (FOR PG STUDENTS)', 'Eg:9.2', 48),
                  _entryFieldnumbers('UG YEAR OF PASSING (FOR PG STUDENTS)', 'Eg:2023', 49),
                  _entryFieldalphabets('UG - COLLEGE OF STUDIES (FOR PG STUDENTS)) ', 'Eg:St.joseph\'s', 50),
                  _entryFieldalphabets('UG - GRADUATING UNIVERSITY) ', 'Eg:Anna University', 51),
                  ...
                  _entryFieldalphabets('ALTERNATE EMAIL ID', 'abc@gmail.com', 57),
                  _DropBox("SPORTS QUOTA", yesorno, 6),
                  _DropBox("BEC EXAM STATUS", yesorno, 7),
                  _DropBox("BEC EXAM GRADE", becgrade, 8),
                  _entryFieldalphabets('LANGUAGES KNOWN', 'Tamil,English,Hindi', 58),
                  _entryFieldnumbers('GAP IN EDUCATION (in Years) - If Any', 'if there is no gap enter 0', 59),
                  ...
                  _entryFieldalphabets('PERMANENT ADDRESS WITH PIN CODE', '', 72),
