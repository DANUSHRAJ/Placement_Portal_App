...
void profile(BuildContext context,List<String> profile,List<NewObject> dropbox) async {
...
//    var tkey=compareList[i].toString();
//    var tvalue=compareList[i].toString();
////    ProfileData res = ProfileData(tkey, tvalue);
//    jsonformat.add({
//      '$tkey':'$tvalue'
//    });
//  }
...
//  log('$result');
  ...
  try {
    ...
    //email id
    if (!RegExp(r'^([a-z0-9\.-]+)@([a-z0-9-]+).([a-z]{2,20})$').hasMatch(
        profile[56])) {
      //print("primary");
    ...
    //pancard
    if (!RegExp(r'[A-Z]{3}[ABCFGHLJPTF]{1}[A-Z]{1}[0-9]{4}[A-Z]{1}$').hasMatch(
        profile[60])) {
      //print("pan card");
